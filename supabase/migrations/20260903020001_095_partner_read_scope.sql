...
--
-- ولماذا في مخطط iam لا في المخطط الذي يستعملها: هذا **نفيٌ** لا منح —
-- «سلطته لا تتعدى الشراكة» — والصلاحية المسمّاة تقول ما يُسمح به لا ما
-- ينقص، فلا تعبّر عنه. ومحلّه عائلة محمولات الهوية نفسها
-- (iam.is_well_partner وiam.has_well_role وiam.has_well_permission).
-- وحرس م-18 (اختبار 082 التحقق 5) يمنع مصفوفة الأدوار النصية في
-- مخططات العمل السبعة تحديدًا — وهي فيها فحص سلطة على عملية، أما هنا
-- فمحمول هوية يقرأ عن المتصل نفسه ولا يفتح له عملًا.
-- ==============================================================
...
--
-- سلطته: **شراكة سارية على هذا البئر** وحدها، عبر iam.is_well_partner.
-- ولا مصفوفة أدوار نصية في جسمه: م-18 أُغلقت بحرس دائم (اختبار 082
-- التحقق 5) يشترط صفر دوال تحملها في مخططات العمل السبعة. والفحص الأول
-- لهذه الهجرة أسقطه فرعُ «أو من يديره» فحُذف — ولا حاجة إليه أصلًا:
-- للمالك عقوده هو (api.list_well_partners وapi.list_well_sessions) وفيها
-- الجلسة الجارية بأرقامها، فحصر هذا العقد بالشريك حصرُ نطاق لا حجب معلومة.
--
-- **وهذا الشرح هنا لا داخل جسم الدالة بقصد:** الحرس يفحص نصّ التعريف
-- بـpg_get_functiondef، والتعليق الذي يشرح النمط الممنوع يسقطه كما لو
-- كان النمط نفسه. الفحص الثاني للهجرة سقط لهذا السبب وحده — وهو نفس درس
-- «النصّ الملغى يُذكر في تعليق فيُفشل مقياسًا نصّيًا» في طبقة الهاتف.
-- ==============================================================
...
begin
  ...
  -- السلطة: شراكة سارية على هذا البئر وحدها — حقيقةُ بيانات لا دورٌ نصّي،
  -- وهي نفس المحمول الذي تقوم عليه كل سياسات اطلاع الشريك في هجرة 050.
  -- auth.uid() لـanon وservice_role = null فالنتيجة false، والإجراء يفشل
  -- مغلقًا. وتفصيل الاختيار في التعليق أعلى الدالة لا هنا.
  if not iam.is_well_partner(p_well_id) then
    raise exception 'ملخص الشريك متاح لشريك سارٍ في هذا البئر وحده'
      using errcode = '42501';
  end if;
...
comment on function finance.read_partner_overview(uuid) is
  'م-41E/4: ملخص الشريك على بئره. يتجاوز RLS في موضع واحد — عدّ الجلسات الجارية للحضور — ولا يعيد أي رقم منها. وسلطته شراكة سارية وحدها، بلا مصفوفة أدوار نصية (م-18 / اختبار 082 التحقق 5).';
