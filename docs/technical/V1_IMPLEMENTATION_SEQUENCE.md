...
2. W1-02 وW1-03 مغلقتان؛ W1-03b مطبقة بق-113 /
   ...
   بق-116 — كلتاهما بلا أي تغيير على قاعدة البيانات؛ أي DB
   change جديد يبدأ Migration 085+.
...
- **W2-02b — لم يبدأ:** WorkManager والإرسال بلا فتح التطبيق،
  والجدولة الزمنية لإعادة المحاولة، وتشغيل العامل بعد إقلاع
  الهاتف، وصلاحيات المانيفست. **محجوب في بيئة المساعد:**
  `workmanager` و`connectivity_plus` غائبة عن `~/.pub-cache`،
  و`androidx.work` غائبة عن Gradle cache، وpub.dev غير قابلة
  للوصول. يفتحه المالك بـ`flutter pub add`.
...
- **W2-02d — لم يبدأ:** Device Readiness وشاشات حالة المزامنة
  ...
  المزارع والأرض والمضخة (بند 20 بند 2).

**م-25 تضيق ثالثةً ولا تُغلق.**
...
**W2-02b — الإرسال الخلفي (WorkManager) والاستعادة بعد إقلاع
الهاتف / بقية م-25. أي DB change جديد = Migration 085+.**
