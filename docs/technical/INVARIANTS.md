...
8أ. **القسمة تجري على كل مقطع جلسة ثم تُجمع النواتج** — لا على
   مجموع الثواني. هذا ما تفعله `Migration 066` فعلًا، وأي حساب
   موازٍ (عدّاد الهاتف، تقرير، شاشة) يجب أن يطابقه. مثال:
   مقطعان 100 ثانية بسعر 3599/ساعة = 99 + 99 = **198**، لا
   `(200×3599)/3600 = 199`. المصدر الحاكم:
   `technical/ACTIVE_SESSION_ARCHITECTURE.md` بند 5 وبند 19،
   والقرار ق-116.
...
هجرة جديدة؛ الهجرات المختومة لا تعدل (السقف: `AGENTS.md` §4).
...
681. الهجرات المختومة تبقى immutable — والسقف ورقم التالي في
     `AGENTS.md` §4 وحده، فلا يُنسخ رقم هنا.

## ق-119 — ثوابت تجربة الاستخدام العالمية والتفقيط ومنع التكرار

682. كل رقم مالي أو هاتفي يُدخل أو يُعرض يجب أن يمر عبر `ArabicToEnglishDigitsFormatter` لتوحيد الأرقام إلى 0-9.

683. الفواصل الألفية للمبالغ والأسعار إلزامية ولحظية أثناء الكتابة عبر `ThousandsSeparatorInputFormatter`.

684. التفقيط المالي العربي بالريال اليمني (`Tafqeet.format`) إلزامي وموحد تحت الحقول المالية.

685. فحص منع تكرار رقم الهاتف حتمي ولحظي عند إضافة الشركاء أو المشغلين أو المزارعين.

686. استيراد بيانات الشخص المسجل مسبقاً (`One-Tap Auto-fill`) يعيد استخدام المعرف الثابت UUID ويمنع تكرار إنشاء الشخص.

687. مبدل الآبار في الشريط العلوي يجب أن يعكس سياق البئر الفعال لجميع العمليات ولا يعتمد على تخمين البئر.

688. دالة `core.setup_well_full` ودوال المخططات الداخلية يجب أن تكون مسحوبة من `PUBLIC` ومحصورة بـ `api.*`.

## م-41D2 / 092 — ثوابت حدّ القراءة المالية ومؤشرات التقارير

689. عقد القراءة يعيد وحدات القاعدة كما هي: المدد بالثواني، والوقود بالمليلتر، والمال `*_minor`. التحويل إلى ساعة أو لتر أو نسبة مئوية أو تسمية عربية تخطيطُ عرضٍ في الشاشة وحدها، ولا يُدفن داخل العقد ولا داخل المستودع.

690. المستودع لا يلتقط خطأ العقد ولا يستبدله ببيانات: الفشل يصعد إلى الشاشة، والشاشة تُظهره صريحًا وتُفرِّغ حالتها. لا بيانات بديلة ولا دوران أبدي ولا `catch (_)` صامت.

691. أسبوع التقارير يبدأ **السبت** محسوبًا صريحًا، لا بـ `date_trunc('week')` الذي يبدأ الاثنين، والحمولة تعلن ذلك في `week_starts_on` حتى لا يُخمَّن في العميل.

692. لا حساب مال جديد داخل عقد قراءة (ق-99): كل مبلغ عمود أو عرض `reporting` قائم. الاستثناء الوحيد المعلَن `remaining_minor` في سطر توزيع الشريك بتعريف 068 نفسه؛ وأي استثناء لاحق يحتاج تسجيلًا مماثلًا لا صمتًا.

693. لا يُوجَّه نداء إلى عقد قبل إثبات وجوده في القاعدة: اسم الدالة وتوقيعها وأسماء الأعمدة تُتحقَّق من الهجرات قبل كتابة سطر في العميل. 41B و41D2 أثبتا أن أسماء وجداول وأعمدة مفترضة كثيرة لا وجود لها أصلًا.

## م-41D7 / 093 — ثوابت سلطة قراءة التسعيرة

694. حق الاطلاع ليس حق التعديل: حين يحتاج دور رؤية بيانات يملكها غيره، تُضاف **صلاحية قراءة مستقلة** (`price.read`) ولا تُوسَّع صلاحية التعديل (`price.manage`) لتشمله. توسيع صلاحية كتابة لأجل قراءة يمنح كتابةً لم يقررها أحد.

695. سلطة القراءة طبقتان تتحركان معًا: الفحص المسمّى داخل الدالة **و** سياسة RLS على الجدول. تخفيف الفحص وحده داخل عقد `INVOKER` يُعيد صفر صفوف فتظهر النتيجة «لا بيانات» — **غياب كاذب** أسوأ من الرفض الصريح، لأن الرفض يقول الحقيقة والغياب يكذب.

696. التجاوز في نقطة واحدة مُراجَعة: القراءة التي تحتاج تجاوز RLS تسكن إجراءً داخليًا `SECURITY DEFINER` بـ`search_path` مثبت يحمل الفحص المسمّى، ويبقى مخطط `api` بصفر `SECURITY DEFINER`. والغلاف لا يكرّر قرار الصلاحية (084 PASS 7): القرار حيث يقع التجاوز.

697. تخفيف بوابة القراءة لا يفتح الجدول: سياسات `SELECT` تبقى كما هي، والاطلاع الموسَّع يمر عبر العقد وحده — فيبقى مسار واحد قابلًا للتدقيق بدل جدول مفتوح لكل قارئ.

698. من يحق له بدء العمل يحق له رؤية شرط تسعيره: مجموعة أدوار قراءة التسعيرة هي نفسها مجموعة الأدوار التي تقبلها `ops.start_irrigation_session`. أي انفصال بين المجموعتين يُنتج مشغلًا يبدأ جلسة مُسعَّرة وهو لا يعرف سعرها.


## 2026-09-02 — ثابت أدوات التحقق

699. أداة التحقق تخضع لق-113 كما تخضع لها الواجهة: أي سكربت يُثبِّت توقعه بقائمة أسماء **مكتوبة يدويًا** لجولةٍ ما يفحص تلك الجولة إلى الأبد ويمرّ على ما بعدها بصمت، فيُعلن نجاحًا كاذبًا يُصادق على البقية. التوقع يُشتق من مُخرَج مولَّد يُعاد بناؤه كل جولة (`docs/technical/db/functions.txt` عبر `npm run db:index`)، وما لا مصدر مولَّد له يُطبع **قياسًا لا حكمًا** ليقارنه إنسان. `scripts/cloud_verify.sh` كان يفحص عقود 092 بعد نشر 093 ويقول «متطابقان».

## م-41D8 / ق-122 — ثوابت الهوية الحقيقية

700. الهوية تُقرأ **مرة واحدة** من `api.app_bootstrap()` وتُمرَّر **وحدةً واحدة** (`AppIdentity`) إلى كل شاشة: صاحب العملية، وآباره، وبئره النشط معًا. لا شاشة تستقبل معرّف بئر ولا اسم بئر ولا مفتاح حساب مفردًا، لأن الحقل المفرد يُملأ بقيمة مكتوبة عند أول شاشة تنساه.

701. `accountId` هو `profile.id` = `auth.uid()` كما يعيده العقد، **وهو نفسه** مفتاح الطابور المحلي. الكتابة بمفتاح والقراءة بآخر تُظهر «لا معلَّق» و«لا جلسة جارية» لعملٍ قائم فعلًا — غياب كاذب في مال المستخدم. فكل عقد يمسّ الطابور يأخذ `accountId` من المُنادي ولا يحمل مفتاحًا ثابتًا في المستودع.

702. حقول الهوية **غير قابلة للإفراغ** حيث لا يُسمح بغيابها: `activeWell` بئر حقيقي إلزامي لا `WellSummary?`. الحقل القابل للإفراغ هو ما يخلق حالة «لا بئر مختار» التي تُملأ باسم جاهز؛ ومنعُ تمثيلها في النوع أقوى من رسالة تحرسها.

703. نتيجة قراءة الهوية **ثلاث حالات معلنة لا رابعة** (`IdentityReady` / `IdentityWithoutWell` / `IdentityUnavailable`): جاهزة، أو حساب بلا بئر، أو تعذُّر مع سببه وإعادة محاولة. ولا يُبنى أي محتوى إلا من الأولى — الدخول إلى شاشات ممتلئة باسم وبئر ودور ليست للمستخدم هو الشكل الأخطر من النجاح الكاذب، لأنه يبدو عملًا سليمًا.

704. القيم الجاهزة للاختبار تسكن **ملف تجهيز واحد في `test/`** (`test/support/identity_fixture.dart`) ولا تُكتب في `lib/`. فصلُ المصدرين هو ما يمنع رجوع `'well-1'` و`'بئر الخير الرئيسي'` و`'active-user'` إلى الشاشات بحجة أن الاختبارات تحتاجها. وحرس الحد يفحص `lib/` وحده، فلو سكنت القيم فيه لفقد معناه.

705. اسم من نفّذ العملية على السند المطبوع اسمه المسجَّل في حسابه. غيابه **يُسقط السطر** ولا يُطبع لقب عام («المشغل») في موضع اسم إنسان: سند بلقب عام لا يُثبت من استلم النقد.

## م-41E / ق-123 — ثوابت الدعوة والتنشيط ونطاق الشريك

706. **لا كلمة مرور يكتبها أحد لأحد.** كلمة المرور التي يعرفها شخصان ليست كلمة مرور، ومالكٌ يكتب كلمات مرور مشغّليه يُفقد النظام القدرة على نسبة عملية نقدية إلى شخص واحد. صاحب الحساب يختارها بنفسه بعد أن يُثبت هويته، دائمًا.

707. **الدعوة بصفر صلاحية حتى يُطالَب بها.** التعيين قبل التنشيط لا يمنح شيئًا، ويُثبت ذلك على **الخادم** لا بإخفاء زرّ في الواجهة. ولها **مدة صلاحية** تنتهي، فرقمٌ أُدخل خطأً لا يبقى قنبلة نائمة تُطالَب بها بعد سنة — والأرقام تُعاد بيعها.

708. **الرمز يُخزَّن مُلبَّدًا بمدة وعدّاد محاولات**، ويموت باستهلاكه أو بانتهائها أو باستنفاد المحاولات. ولا يُكتب نصًّا في جدول ولا في سجل ولا في رسالة خطأ.

709. **ربط الشخص بالبئر يُحلّ عند كل دخول، لا لحظة إنشاء البئر وحدها.** الربط اللحظي يُنتج سجلَّين لا يعرف أحدهما الآخر: دعوة معلَّقة وحساب أُنشئ بعدها لا يجتمعان أبدًا. ومن له حساب قائم يُربط **بلا رمز** — الرمز لإثبات هوية جديدة لا لتكرار إثبات قائم.

710. **شاشة إدخال الرقم لا تفشي من هو مسجَّل:** ردٌّ واحد للمسجَّل وغيره، **ولا تُرسَل رسالة إلا لرقم مسجَّل سلفًا**. هذا يحمي شيئين معًا: سرّية من في النظام، ورصيد الرسائل من هجمة تضخيم الإرسال (`SMS Pumping`).

711. **رمز يُسلَّم باليد أقوى من رمز يُرسل**، لا أرخص فقط: الرسالة تُثبت ملكية الرقم، والمالك الذي يقرأ الرمز لمشغّله أمامه يُثبت **أنه هو من سلّمه**. فالمسار اليدوي أساس والرسالة بديل، ولا يُقلب الترتيب لأجل الأناقة.

712. **الرمز في كل دخول ممنوع كتصميم**: كلفته ربع دولار للرسالة في اليمن، والمشغّل يعمل حيث لا تغطية — فرمزٌ لا يصل يعني **جلسة سقي لا تبدأ**. كلمة المرور هي بيان الهوية الذي يعمل بلا شبكة، والرسائل للتحقّق والاستعادة وحدهما.

713. **حضور الجلسة الجارية حقيقة نهائية، ورقمها ليس كذلك.** الجلسة غير المقفلة لا تدخل مجاميع أي يوم (ق-37)، فمن يُعرَض عليه مستحقّها أو مدّتها يبني توقّعًا ينقلب. لمن لا يملك التشغيل: **الحضور نعم، الأرقام غير النهائية لا**.

714. **إلغاء الوصول ليس حذفًا.** يوقف التعيين ولا يمسّ سجلًّا ماليًّا ولا تاريخ شراكة، فيرى الشخص «لا يوجد بئر مرتبط بحسابك» — حالة معلنة قائمة لا شاشة فراغ.

715. **«لحظي» يُعلَن برقم أو لا يُعلَن.** لا دفع لحظيًّا في هذا الإصدار: القراءة عند فتح الشاشة وبالسحب، ومؤشِّر الجلسة الجارية كل 60 ثانية، ووقت آخر تحديث معروض. وعدُ «فورًا» بلا رقم يقيسه ادّعاءٌ من عائلة النجاح الكاذب.
