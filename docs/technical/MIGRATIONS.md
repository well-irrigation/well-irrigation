...
5. **`finance.read_partner_overview(uuid)`** — `DEFINER` بسلطة صريحة
   (شراكة سارية أو دور يدير البئر). التجاوز لغرض واحد: **عدّ** الجلسات
   الجارية للحضور، لأن السياسة الجديدة تحجب صفّها فيصير العدّ صفرًا
   دائمًا — وذلك غياب كاذب. ولا تعيد معرّف جلسة ولا مستحقًّا ولا مدة ولا
   مضخة. ومعها الفترة المفتوحة موسومة `is_final = false` بمجاميع أعمدة
   مخزَّنة من `reporting.well_daily_summary` بلا صافٍ محسوب.
...
بـ**25 تحققًا**: الجارية محجوبة والمقفلة مقروءة، وتفصيل الجارية مرفوض
...
ولا DML مباشرة، والشريك المشغّل لا يُقيَّد، والمالك بلا انحدار، والغريب
و`anon` مرفوضان.

**الأثر المتوقَّع على الفهرس (يُثبت بـ`db:index` لا يُفترض):** functions
447 → **451**، والأعمدة والقيود والمشغّلات بلا تغيير.
