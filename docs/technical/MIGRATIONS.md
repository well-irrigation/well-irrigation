...
## 098 — 20260904050001_098_day_boundary_timezone.sql

**الغرض:** إنفاذ ثلاثة قرارات نافذة كان عقد التقارير يخالفها، وكلها تصيب
نفس الأرقام التي يقرأها المالك ليقرّر.

**العطب المقيس (2026-09-04):** `core.tenants.timezone` موجود `NOT NULL`
بافتراض `Asia/Aden` **ولا دالة واحدة في القاعدة تقرأه**. فـ092 كان يحسب
الفترات المسمّاة بـ`date_trunc('day', now())` بمنطقة جلسة القاعدة — أي
`UTC` — فيبدأ «اليوم» الساعة 03:00 بتوقيت عدن: جلسة تبدأ 01:30 ليلًا
تُحسب على أمس، ومجموع اليوم ينقص ثلاث ساعات من أوله وتُضاف إليه ثلاث من
أمس. والفترة المخصَّصة تُقصّ بالمنطقة نفسها فتُزاح حتى لو أرسل العميل
حدودًا صحيحة. وإسناد اليوم في السلسلة اليومية (`::date` على `started_at`
و`paid_at` و`spent_at`) وحساب أول الأسبوع كلها بمنطقة الخادم.

**وق-27 وق-37:** ق-27 يقول «الجلسة العابرة لمنتصف الليل تُنسب كاملة إلى
يوم انتهائها ولا تُجزّأ» — وهو **تعديل من المالك** على اقتراح كان يقول «يوم
البداية»، و092 بُني على الاقتراح المرفوض (`started_at`). وق-37 يقول
«الجلسة غير المقفلة لا تدخل مجاميع أي يوم» و092 كان يُدخلها. وق-39 (عبور
الشهر يتبع شهر النهاية) نتيجة مباشرة لق-27 فتحقّق معه. **وق-38 لم يُلمس:**
انتماء الجلسة للنوبة يبقى ببدايتها لمساءلة المشغّل، وانتماؤها المحاسبي
بنهايتها — القرار يفرّق بينهما صريحًا.

**والتدقيق السابق كان قد قاس العيبين وسجّلهما**
(`start_day_sessions=2, end_day_sessions=0, open_sessions_inside_daily=1`،
خلافًا لق-27/ق-37/ق-39) وسجّل أن لا اختبار دائمًا يحرسهما — فنُسي ما لا
يُقاس. وأشدّ من ذلك: **التحقق 28 في اختبار 092 كان يشترط**
`period_start = date_trunc('day', now())` — أي اختبارٌ **يُرسّخ العطب**
ويمنع إصلاحه، كُتب من الكود لا من القرار. حُدِّث في هذه الجولة ليقيس
الخاصية (منتصف ليل بمنطقة الجهة) لا صيغة حسابها.

**ما فيها:**
- `core.well_timezone(uuid)` — `stable`, `security definer`,
  `search_path=pg_catalog, pg_temp`. تعيد منطقة الجهة المالكة للبئر،
  وافتراضها `Asia/Aden`. صلاحية التعريف **لغرض القراءة وحده**: `core.tenants`
  عليه عزل صفوف، فقراءتها بصلاحية المتصل تُعيد صفر صفوف لمن لا يرى صفّ
  الجهة فتسقط إلى الافتراضي **بصمت** — فتختلف نافذة التقرير باختلاف من
  يفتحه، وذلك غياب كاذب لا رفض صريح. والمنطقة إعداد عرض لا معلومة حسّاسة،
  والدالة لا تمنح سلطة ولا تقرأ غير هذا العمود.
- `core.period_bounds(uuid, text, timestamptz, timestamptz) returns tstzrange`
  — `stable`, `invoker`. تحسب حدود `today`/`this_week`/`this_month`/`custom`
  بالصيغة `(date_trunc(..., now() at time zone tz)) at time zone tz`،
  فالحدّ منتصف ليل **محليّ** والأسبوع يبدأ السبت وسقف المخصَّص 92 يومًا.
- `api.get_reports_summary` أُعيد تعريفه عليهما: التصفية والإسناد على
  `ended_at` مع `ended_at is not null`، وإسناد اليوم
  `(x.ended_at at time zone tz)::date`، وكذلك `paid_at` و`spent_at`.
  والمغلَّف صار `version = 2` وكسب `timezone` و`session_day_basis` و
  `open_sessions_excluded` — فأساس الحساب يُعلَن في المخرَج لا يُفترض.

**ما ليس فيها بقصد:** لا تعديل في العميل. الشاشة ترسل رمز الفترة
(`today`/`this_week`/`this_month`) ولا تحسب حدودًا ولا تستعمل `custom`،
فالعطب كان كله على الخادم. ولا دالة `api` جديدة، ولا صلاحية جديدة، ولا
تغيير في بيانات مخزَّنة.

**الاختبار:** `supabase/tests/20260904_098_day_boundary_timezone.test.sql`
بـ**10 تحققات**: خصائص دالة المنطقة و`anon` محجوب؛ وحدّ اليوم منتصف ليل
محليّ بمدى يوم؛ و**النافذة ثابتة حين تُبدَّل منطقة جلسة القاعدة من `UTC`
إلى `America/New_York`** — فلا يتبع التقرير من يقرأ؛ والحدّ يزيح عن منتصف
ليل الخادم؛ ثم س-02 وس-03 مُجسَّدان في جلسة واحدة (31 يناير 23:20 →
1 فبراير 01:40 بتوقيت عدن): نافذة 31 يناير تحمل النهارية وحدها، ونافذة
1 فبراير تحمل العابرة كاملةً 8400 ثانية، والسلسلة اليومية تُسندها إلى
`2026-02-01`؛ وق-37 بجلسة جارية موجودة في اليوم ومستثناة من المجاميع؛
والمغلَّف يعلن المنطقة وأساس اليوم والنسخة 2؛ ورفض `anon` صريحًا.

**الإثبات المحلي (تشغيل المالك 2026-09-04):**
`FILES=37 PASS=603 FAIL=0 ERROR=0`، والفهرس functions 458 → **460**
(الدالتان الجديدتان)، وcolumns 833 وconstraints 501 وtriggers 44 **بلا
تغيير** — لا بنية جديدة ولا قيد.

**تحذير النشر:** **الأرقام المعروضة ستتغيّر، وهذا هو الغرض.** مجموع «اليوم»
