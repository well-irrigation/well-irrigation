...
## م-41C2 — عقود قراءة الجلسات (Migration 090)

سجل الجلسات وتفصيلها كانا الحالة الأسوأ في الحدود: Flutter كان
يقرأ `ops.irrigation_sessions` و`ops.session_segments`
و`billing.session_charges` مباشرة، **وبأسماء أعمدة غير موجودة في
القاعدة أصلًا** (`segment_index`, `duration_seconds`,
`hourly_rate_minor`, `is_paused`, `pause_reason`). فحتى لو كُشفت
المخططات لكان النداء فاشلًا. النتيجة الفعلية: كل فتح للسجل يسقط
إلى `_getMockSessionHistory` / `_getMockSessionDetail`، فتظهر
مبالغ لا وجود لها في القاعدة.

Migration 090 تضيف عقدين:

| العقد | الوسائط | يعيد |
|---|---|---|
| `api.list_well_sessions` | `p_well_id uuid`, `p_farmer_well_account_id uuid default null`, `p_from timestamptz default null`, `p_to timestamptz default null`, `p_unpaid_only boolean default false`, `p_limit integer default 200` | `{contract, version, items[]}` |
| `api.get_session_detail` | `p_session_id uuid` | `{contract, version, session, segments[], payment}` |

الخصائص الستة نفسها من 089 سارية (INVOKER، لا كائنات علائقية في
`api`، تفويض من RLS، حد مثبت وanon محجوب، ترتيب حتمي
`started_at desc, id`، ACL صريح)، مع ثلاث قواعد إضافية تخص هذا
العقد تحديدًا:

**١) لا حساب مال داخل عقد القراءة (ق-99 + القرار 341).**
المبالغ تُقرأ كما خُزّنت: `billing.session_charges.duration_seconds`
و`amount_minor`، و`billing.invoices.paid_minor`، وأعمدة
`ops.session_segments.*_charge_minor`. لا `rate × seconds / 3600`
لا في القاعدة ولا في Flutter.

المدفوع يُشتق بأسبقية صريحة: `invoices.paid_minor` إن وُجدت فاتورة
(068 يصونها عبر التخصيصات)، وإلا مجموع `billing.payments` بحالة
`posted` على تكلفة الجلسة — فلا احتساب مزدوج.

**٢) الجلسة غير المفوترة حالة مستقلة.** لا شيء يحولها إلى صفر
مستحق ولا إلى `settled`: `total_amount_minor` و`paid_amount_minor`
يعودان `null`، و`payment_status = 'not_billed'`، و`has_charge = false`.
الشاشات تعرضها «غير مفوترة بعد»، وتمنع طباعة فاتورة لها.

الحالات الأربع التي يحسمها العقد: `not_billed` / `unpaid` /
`partial` / `settled`.

**٣) النافذة الزمنية وسيط صريح.** عقد حدود اليوم على الخادم ما زال
مسألة مفتوحة (لا منطقة زمنية محسومة في القاعدة)، فلا يجوز اختراعه
داخل العقد. العميل يحسب `p_from` / `p_to` بالتوقيت المحلي للجهاز
في `OperationsRepository.historyWindow` ويرسلهما، و`p_to` حصري
(نصف مفتوح).

رموز `segment_type` و`energy_source` تعود كما هي في القاعدة.
الترجمة العربية مسؤولية طبقة العرض بتخطيط صريح
(`kEnergySourceLabels` و`kSegmentTypeLabels` بأنواعها التسعة)،
والرمز المجهول يظهر كما هو — لا Blind Remap.

كل انضمامات التزويق (الأرض، الحساب، الشخص، المضخة، المشغل) هي
`LEFT JOIN` إجباري: `farmer_well_account_id` قابل للعدم في الجلسة
وفي الأرض، وRLS قد تحجب صف تزويق — وانضمام داخلي كان سيمحو جلسة
مرئية بصمت.

الاختبار الدائم: `supabase/tests/20260831_090_session_read_contracts.test.sql`
(25 تحققًا: التوقيعان وINVOKER+STABLE+search_path وACL، عزل البئر
غير المسند، الترتيب الحتمي، ترشيح المزارع والنافذة و`p_unpaid_only`،
تثبيت الحد، الحالات الأربع للسداد، `total_amount_minor = null`
للجلسة غير المفوترة، اشتقاق `is_stop`، رفض anon، و`42501`/`22023`).
