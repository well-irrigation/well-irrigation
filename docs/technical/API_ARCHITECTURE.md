...
> `from()`. أُغلقت كلها على مراحل: م-41A/B (نداءات RPC)، م-41C1
> (089)، م-41C2 (090)، م-41D1 (091)، م-41D2 (092). العدّ الحالي
> **0 / 0 / 0** ويحرسه `test/core/api/data_api_boundary_test.dart`
> بثلاث قوائم دين فارغة. فالنصوص أدناه تصف العقد الواجب **وهو
> المطبَّق فعلًا** في العميل الآن؛ ويبقى أن اختبار 092 على القاعدة
> لم يُشغَّل بعد، فحالة الهجرة = مكتوبة لا مُتحقَّقة.
...
## م-41D2 — عقود القراءة المالية ومؤشرات التقارير (Migration 092)

هذه الجولة أغلقت آخر حدّ مكسور. قبلها كانت الشاشات المالية تقرأ
`from('schema.table')` على مخططات غير مكشوفة، وكل قراءة ملفوفة
بـ`catch(_)` يعيد بيانات تجريبية؛ ومؤشرات التقارير نداء RPC مجرّد
باسم عقد غير موجود. فالفشل كان يظهر نجاحًا، ومالٌ ثابت في العميل
يُعرض كأنه حساب.

**ما ثبت قبل كتابة العقود** (تطبيقًا لقاعدة 41B: لا يُوجَّه نداء
إلى عقد قبل إثبات وجوده):

- `iam.well_memberships` لا وجود له في القاعدة أصلًا. المصدر
  الحقيقي `core.well_partners` + `core.ownership_share_versions`.
- `billing.invoices.issue_date` لا وجود له؛ العمود `invoice_date`.
- `public.farms` لا وجود له؛ الأراضي `ops.farms`، وربطها بالفاتورة
  عبر `ops.irrigation_sessions.farm_id`.
- محلّلات JSON في العميل كانت تنتظر مفاتيح لا ينتجها جدول خام، أي
  أنها كُتبت لعقد لم يُبنَ. العقود الجديدة تحمل تلك الأسماء نفسها
  فوق أعمدة موجودة، فلا Blind Remap ولا تغيير للعميل بلا سبب.
- أرقام الشركاء والمزارعين **محسوبة أصلًا** في `reporting` منذ 060
  (`partner_account_summary`، `farmer_account_balances`)، والمدفوع
  للشريك عمود حقيقي `paid_minor` من 068.

### العقود

| العقد | الوسائط | يعيد |
|---|---|---|
| `api.list_well_expenses` | `p_well_id uuid`, `p_status text default null`, `p_limit integer default 100` | `{contract, version, items[]}` |
| `api.list_well_partners` | `p_well_id uuid`, `p_limit integer default 100` | `{contract, version, items[]}` |
| `api.list_well_profit_cycles` | `p_well_id uuid`, `p_limit integer default 24` | `{contract, version, items[]}` مع `partner_lines[]` داخل كل دورة |
| `api.get_farmer_account` | `p_farmer_well_account_id uuid`, `p_limit integer default 50` | `{contract, version, account{}, invoices[], payments[]}` |
| `api.get_reports_summary` | `p_well_id uuid`, `p_period text default 'this_month'`, `p_start timestamptz`, `p_end timestamptz` | `{contract, version, period_code, period_start, period_end, week_starts_on, totals{}, daily_irrigation[], financial_trends[], energy_distribution[]}` |

الخمسة `security invoker` + `stable` + `search_path` مثبت، لا
`SECURITY DEFINER` ولا كائن علائقي في `api`. التفويض من RLS
القائمة على `core.wells` و`ops.farmer_well_accounts`، ولم يُخترع
رمز صلاحية للقراءة. fail-closed بـ`28000`/`22023`/`42501`، والحدود
مقصوصة، والترتيب حتمي في كل قائمة وفي كل قائمة داخلية.

`get_farmer_account` **لا يأخذ `p_well_id`**: البئر مشتق من الحساب
نفسه، فإرسال بئر مع حساب لا ينتمي إليه كان بابًا للخطأ. وسندات
القبض تُلتقط بمساريها — دفعة الحساب مباشرة ودفعة الجلسة عبر
تخصيصاتها على فواتير الحساب (قيد 043 يربط الثانية بـ
`session_charge_id` لا بالحساب)، وإلا لظهر سجل ناقص وهو نجاح كاذب
من نوع آخر.

### حدّ ق-99 داخل عقد قراءة

لا حساب مال جديد في العقد: كل مبلغ عمود أو عرض `reporting` قائم.
والتحويل إلى ليتر أو ساعة أو نسبة مئوية أو تسمية عربية أو اسم يوم
مسؤولية طبقة العرض وحدها.

استثناء واحد مقصود: `remaining_minor` في سطر توزيع الشريك يُعاد
كفرق `net_payable_minor − paid_minor` — التعريف نفسه الذي تفرضه
068 داخل `finance.pay_partner_distribution` وتقبل الدفعة أو ترفضها
بناءً عليه. فهو ثابت مجالي لا تجميل. ومقابله: «صافي التدفق» لا
يُعاد من العقد، بل تحسبه الشاشة من رقمين مُعادين، لأنه تجميع عرضي.

### وحدات القاعدة وحدود الزمن

`totals` تعيد المدد **بالثواني** والوقود **بالمليلتر** والمال
`*_minor`. الشاشة وحدها تحوّل: 320000 مل ← «320 لتر»، و86400 ثانية
← «24 ساعة تشغيل»، والنِسَب من الثواني المُعادة.

الفترة رمز يفهمه العقد: `today` / `this_week` / `this_month` /
`custom`. الأسبوع يبدأ **السبت** محسوبًا صريحًا `((dow + 1) % 7)`
لا بـ`date_trunc('week')` الذي يبدأ الاثنين، والحمولة تعلن ذلك في
`week_starts_on` حتى لا يُخمَّن. الفترة المخصصة الناقصة أو
المعكوسة أو الأطول من 92 يومًا مرفوضة بـ`22023` ولا تُصحَّح ضمنيًا.
وحدود اليوم على UTC لأن لا عمود منطقة زمنية في `core.wells` — بند
مفتوح لا سلوك نهائي.

### مطابقة Flutter

- `FinanceRepository`: القراءات الأربع صارت
  `.schema('api').rpc(...)`، وحُذفت مولّدات المحاكاة الأربع ومعها
  هوية المزارع ورصيده المقدَّم الثابتان ومال الشركاء المحشور.
- كشف حساب الشريك الواحد **تصفية محلية** لقائمة الشركاء نفسها، فلا
  عقد ثانٍ له ولا نداء لكل شريك.
- `fetchReportsSummary` في `WellManagementRepository` صار عقدًا،
  و`_getMockReportSummary` = 0.
- **لا `catch` في أي مستودع**: الفشل يصعد إلى الشاشة، وست شاشات
  تعرضه نصًّا عربيًا صريحًا مع تفريغ الحالة بدل الدوران الأبدي أو
  أرقام بديلة.
- حرس الحدود `test/core/api/data_api_boundary_test.dart` صار يحمل
  **ثلاث قوائم دين فارغة** (internal-schema، bare-RPC،
  dotted-from) وثلاثة فحوص إيجابية تثبّت التوجيه عبر `api` وغياب
  المحاكاة وحضور رسائل الفشل الست.

### دَينٌ باقٍ معلَن

- `_AllocateAdvanceDialog` في شاشة الحساب المالي للمزارع يرسل
  `paymentId: 'mock-advance-pay'` وقائمة تخصيصات فارغة إلى
  `allocate_payment`. مسار **كتابة** بمعرّف مُلفَّق، وإصلاحه يحتاج
  عقدًا يعرض سندات الرصيد المقدَّم القابلة للتخصيص + واجهة اختيار،
  وكلاهما توسيع تمنعه ق-120. مسجَّل في `OPEN_ISSUES.md`.
