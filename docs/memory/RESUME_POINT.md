...
- م-25: Android Offline/Background Sync — **ضُيِّقت** بق-114
  (2026-08-22): أساس حماية التكرار على الخادم منفَّذ ومُثبت،
  وطبقة الهاتف تبقى مفتوحة وحرجة.
...
حماية التكرار على الخادم) اكتملت أيضًا. الخيارات المطروحة
على المالك بترتيب أولوية ق-109:

1. **W2-02 — طابور الهاتف / بقية م-25.** المسار الافتراضي:
   الخادم صار يميّز العملية المكرَّرة، لكن لا شيء بعد يُرسل
   معرّفًا ثابتًا. المطلوب: قاعدة محلية دائمة، Outbox مرتّب،
   توليد معرّف العملية **مرة واحدة** لكل عملية ميدانية،
   WorkManager، وحالات المزامنة المرئية. Rule C في ق-109
   يمنع إنتاج معظم Field Screens قبل ذلك.
2. **توسيع نمط ق-114** إلى العقود Offline-capable خارج
   الثماني (ورديات، نقل جلسة، حجوزات، مصروفات، توزيعات) —
   يحتاج Migration 085+ وقد صار النمط مُثبتًا.
3. **دفعة RLS مستقلة (Migration 085+)** لنقل 273 policy
...
**م-25 ضُيِّقت ولم تُغلق** بق-114. الخادم صار يميّز العملية
المكرَّرة ويعيد نفس النتيجة بدل تنفيذها ثانية، للدورة
الميدانية الأولى (8 عمليات).

الباقي كله على طبقة الهاتف: قاعدة محلية دائمة، Outbox مرتّب،
**توليد معرّف العملية مرة واحدة لكل عملية ميدانية** (توليده
لكل محاولة يُبطل الحماية كلها)، عمل خلفي، جاهزية الجهاز،
شاشات الحالة، وتصنيف التعارض.

**م-18 مغلقة** بق-113 من الجولة السابقة.
