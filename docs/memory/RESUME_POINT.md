...
**NEXT = م-41D5 نُفِّذت في 2026-09-02: البند 8 — آخر معرّف مُلفَّق في
مسار كتابة — أُغلق. زر «استخدام الرصيد المقدم» كان يرسل
`paymentId: 'mock-advance-pay'` وقائمة تخصيصات فارغة ثم يعلن نجاح
تسديد؛ صار «الرصيد المقدم (غير متاح)» ونافذته `StatelessWidget` بلا
مستودع تعلن أن لا أمر أُرسل وأن الرصيد والدين لم يتغيّرا. دين
المعرّفات المُلفَّقة **1 ← 0** (القائمة فارغة)، والحرس 21 ← **22/22**،
والحزمة 292 ← **294/294 PASS**، و`flutter analyze` نظيف. البوابة ق-120
تبقى مفتوحة على: عقد قراءة سندات الرصيد المقدَّم (به يعود الزر
للعمل، والقرار 420 مفتوح حتى ذلك)، وتسعيرة الساعة في العميل
(3500 / 5000 — مُجمَّعة ومُثبتة لا مُغلقة)، ودين الهوية
(`'well-1'` 45/14، `tenant-1` 13، `'active-user'` 8).
التالي: جولة التسعيرة الحقيقية، ثم جولة الهوية الحقيقية، ثم تقرير
إغلاق ق-120**
...
- **دين مُعلَن لا مُغلَق:** تسعيرة الساعة (3500 / 5000) ما زالت محسوبة
  في العميل بدل `api.get_active_price_schedule`؛ جُمعت في ثابتين
  ودالة واحدة ومُثبتة في الحرس بعدّ واحد لكل رقم. والمنسق يحمل الرقمين
  نفسهما في `PricingResolver` الافتراضي، فتُغلق الجولة الموضعين معًا.
