...
**NEXT = م-41D6 نُفِّذت في 2026-09-02: تسعيرة العميل أُغلقت. شاشة
التشغيل كانت تعرض زرّين ثابتين (3,500 للشمسي و5,000 لـ«ديزل شامل»
الجامعة لمصدرين مختلفَي السعر) وتحسب منهما المستحق الحيّ وسند القبض،
وترسل نصًّا عربيًّا في `p_energy_source` ترفضه هجرة 066 بـ`22023`؛
فصار السعر من `api.get_active_price_schedule` وحده، وخيارات المصدر من
`kSessionEnergySources` (مصادر هجرة 066 الثلاثة) لأن اختيار المصدر قرار
تشغيلي، والغياب يُعلَن نصًّا معتمدًا (القرار 341) لا صفرًا. والتسعيرة
لا تحجب البدء: `price.manage` للمالك وحده (080/091) بينما
`ops.start_irrigation_session` لا تأخذ سعرًا وتُسعّر خادميًّا، فرفض
`42501` حالة صلاحية معلنة لا فشل ولا منع تشغيل. والمنسّق بلا أسعار
افتراضية (`PricingResolver.none()`) ومعه `updatePricing` تُغذّيه من
العقد. لا Migration جديد — العقد موجود منذ 091. الحرس **22/22** ومقياس
التسعيرة انقلب من إثبات وجود `_clientSideRateFor` إلى إثبات غيابه،
والحزمة 294 ← **300/300 PASS**، و`flutter analyze` نظيف. البوابة ق-120
تبقى مفتوحة على: عقد قراءة سندات الرصيد المقدَّم (به يعود زر الرصيد
للعمل، والقرار 420 مفتوح حتى ذلك)، ودين الهوية (`'well-1'` 45/14،
`tenant-1` 13، `'active-user'` 8)، والطابور الدائم غير الموصول، وإظهار
التسعيرة للمشغل (صلاحية قراءة جديدة = Migration تمنعه ق-120).
التالي: جولة الهوية الحقيقية، ثم تقرير إغلاق ق-120**
...
- ضابط حماية `main` كان آخر مرة **غير محفوظ** (`Create` لم يُضغط)،
  وPR #12 (توثيقي) لم يُدمج.
...
3. حماية فرع `main` في `GitHub`: الضابط موجود بإعداداته لكن
   `Create` لم يُضغط، فهو غير نافذ. حاجة **أمنية** لا تنظيمية.
...
- **دين مُعلَن لا مُغلَق:** `farmer_diesel` بنموذج `fuel_based` لا سعر
  ...
  والمشغل لا يرى التسعيرة أصلًا لأن `price.manage` للمالك — إظهارها له
  صلاحية قراءة جديدة = Migration تمنعه ق-120.
