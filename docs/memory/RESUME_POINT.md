...
- م-25: Android Offline/Background Sync — **ضُيِّقت مرتين:**
  بق-114 (2026-08-22) أساس حماية التكرار على الخادم منفَّذ
  ومُثبت، وبق-115 (2026-08-23) طابور الجهاز الدائم ومعرّفات
  العمليات الثابتة منفَّذة. **الإرسال الخلفي والاستعادة بعد
  الإقلاع والشاشات والواجهات الميدانية تبقى مفتوحة وحرجة.**
...
الدائم / ق-115) اكتملت — بانتظار تشغيل المالك للتحقق.**
الخيارات المطروحة بترتيب أولوية ق-109:

1. **W2-02b — الإرسال الخلفي.** المسار الافتراضي: العملية
   صارت محفوظة على الجهاز ولا تُرسل مرتين، **لكن الإرسال
   يحتاج أن يكون التطبيق مفتوحًا**. المطلوب: WorkManager،
   ومراقب اتصال، وجدولة إعادة زمنية، والاستعادة بعد إقلاع
   الهاتف، وصلاحيات المانيفست. لا تغيير على قاعدة البيانات.
2. **W2-02c — سجل الجلسة النشطة** واستعادتها بعد موت العملية.
3. **W2-02d — جاهزية الجهاز وشاشات حالة المزامنة** (ق-90 /
   UX-10). النصوص والحالات موجودة في الطابور بلا شاشة تعرضها.
...
(WorkManager ومراقب اتصال وجدولة إعادة)، والاستعادة بعد إقلاع
الهاتف، وسجل الجلسة النشطة، وجاهزية الجهاز، وشاشات الحالة،
وعرض التعارض، وأي واجهة ميدانية تُدخل العمليات أصلًا.
