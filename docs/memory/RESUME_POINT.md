...
معلَّق: `db:reset` + `db:test` عند المالك (المتوقع 31 files / 500 PASS)،
والدمج في `main` ينشر الهجرة على الإنتاج تلقائيًا (توسيع اطلاع).
...
- **الاختبار:** `20260902_093_price_read_for_operators.test.sql` بـ**15
  ...
  الكتابة.
- **التحقق:** **معلَّق** — `db:reset` + `db:test` عند المالك، والمتوقع
  31 files / 500 PASS. **صفر تغيير في Flutter**، فالحزمة تبقى 300/300.
