# سجل نقاط الاستئناف — أرشيف

**الحالة:** أرشيف · **أُنشئ:** 2026-09-03 · **القرار الحاكم:** ق-93

هذا الملف ليس نقطة العمل. نقطة العمل الحالية وحدها في
`docs/memory/RESUME_POINT.md`، وهو رأس قصير يُقرأ في كل جلسة.

ما تحته هنا هو **السرد الزمني** لكل ما سبق حتى 2026-09-02، منقولًا
حرفًا بحرف بلا حذف: الجولات، والأدلة، والأرقام في تواريخها. يُقرأ عند
الحاجة إلى تاريخ قرار أو سبب رقم، لا في كل جلسة.

القواعد الحيّة و«ما لا يجوز إعادة عمله» والبنود المؤجَّلة **لم تُنقل
إلى هنا** — بقيت في الرأس لأنها ليست تاريخًا.
...
وهدف 093 = 15/15؛ ودُمج `PR #20` مضغوطًا في `main` عند `787eee0` فنُشرت
الهجرة على الإنتاج؛ والتحقق السحابي أثبتها: `MISSING_IN_CLOUD=0` و
`FUNCTIONS_MISSING_IN_CLOUD=0` و`IAM_PERMISSIONS=42` /
`IAM_ROLE_PERMISSIONS=78`. وفي أثناء ذلك ظهر عطب في أداة التحقق نفسها:
`scripts/cloud_verify.sh` كان يثبّت عقود 092 الخمسة بالاسم فأعلن النجاح
بلا أن ينظر إلى 093 — فصار يشتق التوقع من الفهرس المولَّد
`docs/technical/db/functions.txt` (424 اسمًا، 52 في `api`)، ودُمج
الإصلاح مضغوطًا في `PR #21` عند `8123448`. وبعدها أُنشئ باب دخول
الوكلاء `AGENTS.md` في جذر المستودع (و`CLAUDE.md` مؤشِّرًا إليه) لأن
الجذر كان بلا أي ملف تعليمات، فنُقل إليه ما كان يعيش في المحادثة وحدها:
عقد «من ينفّذ ماذا»، وحدّ ٦٠ محرفًا للأمر، وحقائق القناة السحابية،
وأن دمج `main` = نشر إنتاج؛ وأُزيل تقادم `AI_HANDOFF_PROTOCOL.md`
(071–093 و094+ وفرع فطلب دمج بدل Push مباشر) — ق-93 §10.
...
- **الاختبار:** `20260902_093_price_read_for_operators.test.sql` بـ**15
  ...
  الكتابة.
- **التحقق:** ✅ **مثبت 2026-09-02** — محليًا: هدف 093 = **15/15**،
  والحزمة **FILES=31 PASS=500 FAIL=0 ERROR=0**، و`db:index` أعاد
  التوليد. **صفر تغيير في Flutter**، فالحزمة تبقى 300/300. ثم دُمج
  `PR #20` مضغوطًا (`787eee0`) فنُشرت الهجرة على الإنتاج، وسحابيًا:
  `MISSING_IN_CLOUD=0`، `FUNCTIONS_INDEX=424` مقابل `FUNCTIONS_CLOUD=426`
  بـ`FUNCTIONS_MISSING_IN_CLOUD=0`، و`IAM_PERMISSIONS=42` /
  `IAM_ROLE_PERMISSIONS=78`.
- **درس الأداة:** أول تشغيل سحابي أعلن «نجح» وهو لم يفحص 093 أصلًا —
  كتلة العقود في `scripts/cloud_verify.sh` كانت قائمة أسماء **مكتوبة
  يدويًا** لهجرة 092. إضافة أسماء 093 كانت ستؤجّل العطب جولةً، فنُزعت
  القائمة: الفحص الآن يشتق كل `schema.function` من الفهرس المولَّد
  ويقارنها بـ`pg_proc` السحابية بنفس مُرشِّح المخططات؛ الناقص يُفشل
  والزائد يُطبع؛ وأرقام كتالوج الصلاحيات تُطبع **قياسًا لا حكمًا** لأن
  تثبيت 42/78 بالرقم كان سيُعيد التقادم نفسه.
