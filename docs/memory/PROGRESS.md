...
الدالة الحافة **لم تُنشر ولم تُشغَّل مرة واحدة**: لا محليًّا ولا سحابيًّا.
فالاستعادة من طرف إلى طرف غير مُثبتة، والشاشة تعلن «غير متاح» حتى النشر.
ولا تشغيل على جهاز أندرويد حقيقي.

NEXT = إعادة `c:db`، ثم فرع ودمج، ثم نشر الدالة الحافة، ثم `cloud:verify`.
