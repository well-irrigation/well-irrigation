...
## 2026-09-03 — م-41E المرحلة 1: إزالة الأكاذيب الأربعة في المصادقة

### ما ثبت بالتشغيل

- `npm run c:app`: `STEP 1 analyze OK issues=0` و
  `STEP 2 test OK PASS=317 FAIL=0` و`RESULT=SUCCESS`. كانت 313.
- **حرس الحد 26/26 PASS** (كان 24): مقياسان جديدان — «باب المصادقة لا
  يعلن جلسة لم تُنشأ» و«تغيير الرقم يُعلن عدم توفره ولا يُمثَّل».
- **أربعة اختبارات سلوكية جديدة** في `profile_security_screen_test.dart`:
  الرقم يُعلن عدم توفره ولا حقل مدخل في النافذة؛ وكلمة المرور الحالية
  **تصل المستودع فعلًا** (يُقاس بما استلمه لا بما عُرض)؛ ورفضها يُعرض
  رسالته الخاصة ولا يُحفظ شيء؛ والخانة الفارغة تُمنع قبل أي نداء.
- **بلا هجرة وبلا استثناء من ق-120**: لا شيء يمسّ القاعدة، وأرقامها تبقى
  92 هجرة و`FILES=31 PASS=500`.

### غير مثبت

لا تشغيل على جهاز أندرويد حقيقي. ومسار الدخول لم يُختبر أمام خادم حقيقي
بانقطاع شبكة فعلي — المُثبَت أن الكود لم يبقَ فيه مسار يعلن نجاحًا بلا
جلسة، وأن الجلسة تُقاس قبل الإعلان.

NEXT = المرحلة 2 — الدعوة والتنشيط على الخادم (هجرة 094).

## 2026-09-03 — م-41E المرحلة 2: هجرة 094 دعوات الفريق والتنشيط

### ما ثبت بتشغيل المالك

- `npm run c:db`: `STEP 1 db:reset OK` ثم
  `STEP 2 db:test OK FILES=32 PASS=522 FAIL=0 ERROR=0` ثم
  `STEP 3 db:index OK` و`RESULT=SUCCESS`. كانت 31 ملفًا و500 تحققًا.
- **هدف 094 = 22/22** تحققًا، أقواها: دعوة لم تُطالَب بها **صفر صلاحية
  مُثبتًا على الخادم** (لا صف في `core.well_assignments`)؛ ورمز خاطئ
  يخصم من العدّاد **ويبقى الخصم** بعد انتهاء النداء؛ ومطالبتان بالرمز
  نفسه = **تعيين واحد**؛ ومن له حساب قائم يُربط **بلا رمز**؛ والدعوة
  المنتهية تُوسم منتهية ولا تُطالَب بها؛ ومن لا يملك `team.manage` لا
  يدعو ولا يقرأ الفريق؛ وإلغاء الوصول **لا يحذف صفًّا**؛ وقراءة الفريق
  لا تعيد رمزًا ولا تلبيدة.
- **الفهرس المولَّد نما مطابقًا للتصميم حرفيًّا:** columns **800 → 819**،
  constraints **481 → 491**، functions **437 → 447**، triggers **44 بلا
  تغيير**. تسعة عشر عمودًا هي أعمدة الجدول الجديد، وعشر دوال هي ستّ
  داخلية وأربعة أغلفة `api`. تطابق الأرقام دليل مستقل على أن ما وصل
  القاعدة هو ما كُتب.
- **السلطة:** الكتالوج **42 → 43** (`team.manage`) والمنح **78 → 79**
  للمالك وحده. وحُدّثت أرقامها في ثلاثة حروس: 080 و081 **و093**.
- الحزمة على الهاتف **بلا تغيير: 317 PASS** — لا سطر Flutter في هذه
  المرحلة، والعقود تُستعمل في المرحلة 3.

### غير مثبت

**لم يُنشر سحابيًّا:** الدمج في `main` هو ما ينشر الهجرة، ويُتحقَّق بعده
بـ`npm run cloud:verify`. ولا تشغيل على جهاز أندرويد حقيقي.

NEXT = المرحلة 3 — شاشة الفريق عند المالك ومسار التنشيط عند العضو.

### النشر السحابي لهجرة 094 — مثبت 2026-09-03

`PR #27` دُمج مضغوطًا في `main` عند `6edf4af`، ثم `npm run cloud:verify`:
`MIGRATIONS_LOCAL=93` / `MIGRATIONS_CLOUD=93` / `MISSING_IN_CLOUD=0`،
و`FUNCTIONS_INDEX=434` / `FUNCTIONS_CLOUD=436` /
`FUNCTIONS_MISSING_IN_CLOUD=0`، و**`IAM_PERMISSIONS=43` /
`IAM_ROLE_PERMISSIONS=79`**.

الرقمان الأخيران هما الدليل الحقيقي: صفّ الهجرة قد يوجد سحابيًّا وقد لا
يكون أثرها وصل البيانات. وفرق عدّ الدوال (434 مقابل 436) هو نفسه المعلوم
منذ 093: `public.max` و`public.min` من إضافة مثبَّتة سحابيًا لا محليًا.

وأثرها على الإنتاج **قدرة نائمة**: العقود الأربعة لا يناديها التطبيق بعد،
فلا مستخدم يرى فرقًا حتى المرحلة 3.

## 2026-09-03 — م-41E المرحلة 3: شاشة الفريق ومسار التنشيط

### ما ثبت بالتشغيل

- `npm run c:app`: `analyze OK issues=0` و`test OK PASS=328 FAIL=0` و
  `RESULT=SUCCESS`. كانت 317.
- **حرس الحد 27/27** (كان 26): مقياس تاسع لعقود الفريق ومسار التنشيط —
  أربعة عقود عبر `api` وحده، ولا `.from(` في المستودع، وغياب ادّعاء
  «غير متاحة»، ووجود خطوة تأكيد الرقم ونافذة الرمز-مرة-واحدة، و**أن
  `onActivated` يُنادى من موضع واحد** داخل فرع نجاح المطالبة.
- **ستة اختبارات لشاشة الفريق:** تعرض ما أعاده العقد؛ وفشل العقد يُعلن
  ولا يعرض أعضاء بديلين ولا زرّ إضافة؛ والدعوة تمرّ بخطوة تأكيد **ولا
  تُرسل قبلها** (يُقاس بأن سجل النداءات فارغ)؛ ورقم له حساب قائم يُربط
  بلا نافذة رمز؛ وفشل الدعوة لا يعرض رمزًا؛ والإلغاء يُرسل المفتاح
  الحقيقي بعد تأكيد.
- **ستة اختبارات لشاشة التنشيط:** الاسم يُرسل كما كتبه صاحبه؛ ورمز خاطئ
  يعرض المتبقّي ولا يُعلن تنشيطًا؛ وإعادة المحاولة **لا تُنشئ حسابًا
  ثانيًا** (يُقاس بعدّاد إنشاء الحساب = 1)؛ وحساب قائم بكلمة مختلفة يُقال
  صريحًا؛ ولا دعوة سارية حالة معلنة؛ و**نجاح النداء بلا جلسة ليس تنشيطًا**.

### غير مثبت

لا تشغيل على جهاز أندرويد حقيقي، ولا تشغيل أمام خادم حقيقي: العقود
مُختبرة بمزدوجات تُسجّل ما وصلها. والقاعدة بلا تغيير في هذه المرحلة
(`FILES=32 PASS=522`).

## 2026-09-03 — م-41E المرحلة 4: نطاق قراءة الشريك

### ما ثبت فعلًا (طبقة الهاتف، تشغيل الوكيل)

- `flutter analyze` على `lib` و`test`: `No issues found!`
- `flutter test`: **328 → 342 PASS**، `FAIL=0`.
- حرس الحد `data_api_boundary_test.dart`: **27 → 28 مقياسًا**.
- **عشرة اختبارات لشاشة الشريك** بمزدوجات مستودعات: النصيب يُعرض من
  الحمولة كما هي؛ والحضور يُعلن بعدده ومعه سبب حجب الأرقام؛ ولا جلسة =
  حالة تُقال؛ والفترة غير المقفلة موسومة «أرقام غير نهائية»؛ ونسبة الفترة
  المُقفلة **45%** تُعرض إلى جانب نسبة اليوم **60%** فلا تُعاد كتابة
  التاريخ بنسبة الحاضر؛ والشركاء بنِسبهم واسم القارئ موسوم «أنت»؛
  والمصروف بلا اسم مسجِّله؛ والمزارعون بديونهم والوقود بلترات خزانه؛
  وغياب سطر الشراكة يُقال ولا يُملأ بصفر؛ وفشل القراءة يُعلَن بسببه ولا
  يُعرض رقم مكانه؛ **ولا `FilledButton` ولا `ElevatedButton` ولا
  `TextField` ولا `Switch` في الشاشة كلها**.
- **ثلاثة اختبارات للدور:** شريك بلا دور تشغيلي = اطلاع فقط؛ ومن له دور
  تشغيلي إلى جانب شراكته لا يُقيَّد؛ والدور يتبدّل مع البئر النشط.

### ما ثبت فعلًا (القاعدة، تشغيل المالك)

`db:reset OK` ثم `FILES=33 PASS=548 FAIL=0 ERROR=0` (كانت 32 ملفًا و522
تحققًا)، ثم `db:index OK` بنمو مطابق للتصميم حرفيًّا: **functions 447 →
451** (أربع دوال: `iam.is_partner_only` و`finance.read_partner_overview` و
`api.read_partner_overview` و`api.list_well_farmer_balances`)، و**columns
819 وconstraints 491 وtriggers 44 بلا تغيير** — لا جدول جديد ولا عمود ولا
قيد: الجولة تضييق اطلاع وإضافة عقود قراءة، لا توسيع بنية.

**ثلاث تشغيلات فشلت قبله، وكلها في أداة التحقق لا في السلوك:**

١) `082` التحقق 5: القارئ الداخلي كان يقبل `iam.has_well_role` ليرى المالك
ما يراه شريكه، وحرس م-18 يشترط صفر دوال على مصفوفات الأدوار النصية في
مخططات العمل السبعة. الفرع **حُذف** — لا نُقل إلى مخطط غير محروس — وأُضيف
تحقق داخل اختبار 095 يعيد فحص الحرس نفسه.

٢) **نفس الحرس مرة ثانية:** `pg_get_functiondef` يعيد نصّ التعريف
**بتعليقاته**، والتعليق الذي يشرح الحذف كان يحمل اسم النمط الممنوع فعُدّ
كأنه هو. الشرح انتقل فوق الدالة لا داخلها. وهذا رابع موضع لنفس الدرس بعد
«لا بئر مختار» و«تم إرسال رمز التحقق» و«إدارة الفريق غير متاحة».

٣) **خطأ تجهيز:** دورة توزيع أُنشئت **معتمدة** ثم أُضيف بند داخلها، وزناد
`prevent_approved_line_change` (هجرة 052) يمنعه بحق. صارت الدورة محسوبة
غير معتمدة، والاعتماد ليس محلّ القياس هنا.

**ونُشرت 2026-09-03 بدمج PR #29:** سحابيًّا 94/94 و`MISSING_IN_CLOUD=0` و
`FUNCTIONS_INDEX=438` مقابل `FUNCTIONS_CLOUD=440` و
`FUNCTIONS_MISSING_IN_CLOUD=0`، و`IAM_PERMISSIONS=43` /
`IAM_ROLE_PERMISSIONS=79` **بلا تغيير**. وأول قياس بعد الدمج بثوانٍ أظهر
سطر تسجيل الهجرة ناقصًا ودوالّها كاملة — يُعاد القياس بعد دقيقتين.

### غير مثبت

لا تشغيل على جهاز أندرويد حقيقي، ولا أمام خادم حقيقي: عقود الشريك مقيسة
بمزدوجات تُسجّل ما وصلها، وحمولاتها في الاختبار مكتوبة بشكل ما يعيده
العقد فيسقط الاختبار عند أي تغيّر في مفتاح.

NEXT = تشغيل `c:db` لهجرة 095، ثم طلب دمج، ثم `cloud:verify` بعده.

## 2026-09-03 — م-41F: استعادة كلمة المرور بإثبات بشري

### ما ثبت فعلًا (طبقة الهاتف، تشغيل الوكيل)

- `flutter analyze` على `lib` و`test`: `No issues found!`
- `flutter test`: **342 → 354 PASS**، `FAIL=0`.
- حرس الحد: **28 → 29 مقياسًا**. المقياس الجديد يثبّت أن المسار الوحيد
  للاستعادة هو الطرف الخادمي (لا `rpc` ولا `from` في الشاشة)، وأن شاشة
  المالك **لا تحمل كلمة `password` إطلاقًا** — فلا حقل ولا وسيط لكلمة مرور
  عند من لا يملك كتابتها (706)، وأن الدالة الحافة لا تسجّل رمزًا ولا كلمة
  مرور في أي سجل.
- **سبعة اختبارات لشاشة الاستعادة:** الرمز باليد معلَن؛ والنجاح بعد نتيجة
  الخادم وحدها؛ ورمز خاطئ يعرض المتبقّي ولا يُعلن تغييرًا؛ ولا تذكرة =
  حالة تُقال مع ما يفعله المستخدم؛ و«استُهلك ولم يُطبَّق» يقول إن القديمة
  لم تتغيّر؛ وفشل النقل يُعلَن بسببه؛ والمدخلات تُفحص قبل أي نداء.
- **أربعة اختبارات لشاشة المالك:** التأكيد يعرض الرقم؛ والرمز يُعرض مرة
  واحدة بلا رسالة؛ و«لا عضو بهذا الرقم» يُقال بلا رمز؛ وفشل الإصدار يُعلَن؛
  والرموز السارية تُعرض بحالتها ومحاولاتها.

### القاعدة — مثبتة بتشغيل المالك

`FILES=34 PASS=567 FAIL=0 ERROR=0`، وcolumns 819 → **833**، constraints
491 → **501**، functions 451 → **457**، triggers **44 بلا تغيير**.

وتشغيلان فشلا قبلها، وكلاهما في أداة التحقق: `074` التحقق 2 يشترط تناظر
منح سطح `api` بين دور التطبيق ومفتاح الخدمة، وغلاف الاستهلاك كان ممنوحًا
لمفتاح الخدمة وحده. أُعيد التناظر ونُقل الحدّ إلى الإجراء الداخلي (لمفتاح
الخدمة وحده) وإلى رفض صريح لكل جلسة مستخدم في الغلاف — مُثبتًا بالتشغيل.

### النشر — مثبت 2026-09-03 (PR #31)

سحابيًّا: 95 محليًّا مقابل 95 سحابيًّا و`MISSING_IN_CLOUD=0`، ودوال
`FUNCTIONS_INDEX=444` مقابل `FUNCTIONS_CLOUD=446` بلا نقص، و
`IAM_PERMISSIONS=43` / `IAM_ROLE_PERMISSIONS=79` **بلا تغيير**.
**والدالة الحافة `reset-password` نُشرت بنجاح من أول محاولة** — وكانت
الخطوة الوحيدة غير المُجرَّبة في الجولة.

### غير مثبت

**المسار من طرف إلى طرف لم يُجرَّب مرة واحدة على الإنتاج:** لا تذكرة حقيقية
أُصدرت، ولا كلمة مرور أُعيد تعيينها، ولا دخولٌ بكلمة جديدة. كل طبقة مُختبرة
وحدها والمجموع منشور لا مُبرهن. ولا تشغيل على جهاز أندرويد حقيقي.

NEXT = عقد قراءة سندات الرصيد المقدَّم (هجرة 097)، ثم بقية طابور ق-120.

## 2026-09-04 — م-41G: سندات الرصيد المقدَّم والزرّ الذي عاد

### ما ثبت فعلًا

- **القاعدة (تشغيل المالك):** `FILES=35 PASS=577 FAIL=0 ERROR=0`، والفهرس
  functions 457 → **458**، وcolumns 833 وconstraints 501 وtriggers 44 **بلا
  تغيير** — قراءة صافية بلا بنية جديدة.
- **الهاتف (تشغيل الوكيل):** `analyze` نظيف، والحزمة **354/354 PASS**،
  وحرس الحد **29** — والمقياس السادس أُعيد كتابته: كان يحرس حالة «غير
  متاح»، فصار يحرس **شكل ما يُرسل** (معرّف سند من العقد، ومبلغ من الحقل،
  ولا `min`/`clamp`/طرح في الشاشة، ولا معرّف نصّي ثابت).
- **اختبارا الشاشة الجديدان:** النافذة تعرض السندات بمتبقّي كل واحد وتُخفي
  المستنفَد؛ والضغط على «تسديد» بلا اختيار **لا يُرسل نداءً** ويقول ما
  ينقص؛ ثم بعد اختيار السند والفاتورة وكتابة المبلغ يُرسل **معرّف السند
  كما أعاده العقد** والمبلغ المكتوب حرفيًّا — يرصدهما جاسوس في الاختبار.

### فرضية سقطت قبل أن تُبنى عليها صلاحية

كُتب التصميم أولًا بإجراء متجاوز لـRLS لأن الوثائق تقول إن دفعة المقدَّم
محجوبة (سياسة 016). وأول تشغيل أسقط الفحص الذي يُثبت الحجب: هجرة 085 كانت
قد أصلحته للمالك والمشغّل. فحُذف التجاوز وصار العقد `INVOKER` يقرأ تحت عزل
المتصل. **الوثيقة القديمة كانت ستُنتج صلاحية أوسع من اللازم**، والاختبار
الذي يُثبت الفرضية قبل البناء عليها هو ما منعها.

### غير مثبت

لم يُنشر بعد (الدمج هو ما ينشر)، ولا تشغيل على جهاز أندرويد حقيقي.

## 2026-09-04 — ق-120/4: اختبار المسار الكامل والفحص التلقائي

### ما ثبت فعلًا

- **اختبار مسار كامل واحد** `supabase/tests/20260904_e2e_full_cycle.test.sql`:
  مالك جديد → جهة وبئر → تسعيرة نافذة → مضخة → مزارع → مزرعة → جلسة شمسية
  ساعتين → إتمام → فاتورة → دفعة أكبر منها → قراءة السند → تخصيص → مراجعة
  الباقي → حجب غير المصدَّق. **16 تحققًا، كلها PASS من أول تشغيل.**
- **لا تجاوز في التجهيز:** الملف لا يُدخل صفًّا بيده إلا في `auth.users`
  (فالتسجيل يجري في Supabase Auth خارج SQL)، وكل ما بعده يمرّ عبر عقود
  `api` بدور `authenticated`. فمخرَج كل عقد هو مدخل التالي — وهذا ما لا
  يقيسه أي اختبار وحدة: أن السلسلة **موصولة** لا أن كل حلقة سليمة وحدها.
- **الأرقام تُطابَق لا تُصدَّق:** ساعتان × 50000 = 100000 تكلفةً، ودفعة
  150000 تُبقي 50000 بعد التخصيص. والتحققان اللذان يقرآن حقيقة البيانات
  (سطر التكلفة وسطر التخصيص) يُنفَّذان بعد `reset role`: لو قُرئا بعين
  المالك لكان حجبُ سياسةٍ صفرًا يُسمّى «لم يُكتب» — غياب كاذب لا فشل.
- **Baseline:** القاعدة `FILES=35 PASS=577` → **`FILES=36 PASS=593`**
  (+16 = عدد تحققات الملف بالضبط)، والفهرس **بلا تغيير**
  (columns 833 / constraints 501 / functions 458 / triggers 44) — لأن
  الاختبار لا يضيف بنية.

### نجاح كاذب في أداة الجلسة الأولى — أُصلح

`scripts/check_state.sh` كان يطبع `DIRTY_TRACKED=0` و`RESULT=SUCCESS` حين
يعجز `git` عن العمل أصلًا (ملف تهيئة محجوب في مجلد عمل معزول): مخرَج `git`
فارغ، و`wc -l` = 0، فيُقرأ **الصمت صفرًا**. أي أول أمر في كل جلسة كان
يشهد بنظافة شجرة فيها تغيير غير محفوظ. الآن يُفصل العجز عن النظافة: حقول
`git` الخمسة تُعلن `BLOCKED`، والنتيجة `RESULT=BLOCKED_GIT`، وأرقام
الملفات (الهجرات والاختبارات والفهرس) تبقى حقيقية لأنها من نظام الملفات لا
من التاريخ. مُثبت بالتشغيل في بيئة محجوبة. الثابت 699 / ق-113.

### الفحص التلقائي — مكتوب لا مُثبت

`.github/workflows/checks.yml`: مسارَان متوازيان عند كل `pull_request`
وعند الدفع إلى `main` — `c:app` (بـFlutter 3.47.0 المثبَّت على إصدار
المالك لا على قناة متحركة) و`c:db` كاملًا، ثم **حرس تقادم الفهرس**:
`git diff --exit-code -- docs/technical/db` يفشل إن لم يطابق المولَّد
القاعدة. ويُنادي سكربتات المستودع نفسها لا قائمة أوامر موازية تتقادم بصمت.

### غير مثبت

الفحص التلقائي لم يجرِ مرة واحدة (يجري أول مرة على أول طلب دمج)، ولا
**يمنع** الدمج حتى يُضاف إلى شرط حماية `main` في إعدادات GitHub — عملٌ في
الموقع لا في المستودع. ولا تشغيل على جهاز أندرويد حقيقي.

### أول تشغيل للفحص التلقائي — والمقياس النصّي يسقط للخامسة

`PR #34`: مسار القاعدة **اجتاز في بيئة نظيفة** (بناء من الصفر + 36 اختبارًا
+ حرس تقادم الفهرس) — أول برهان للمشروع خارج جهاز المالك. ومسار التطبيق
فشل قراءةً لا موضوعًا: `flutter test` يختار مُخرِجه بحسب البيئة، فيطبع في
GitHub Actions `🎉 354 tests passed.` وفي الطرفية `+354: All tests passed!`.
والأداة تبحث عن الجملة الثانية، فأعلنت `UNKNOWN` لحزمة **ناجحة كلها**.
وهذا خامس ظهور لعائلة «المقياس النصّي يصطدم بصيغة لم تُتوقَّع» — وأولها في
أداة تحقق تقرأ أداة أخرى. والعلاج طبقتان: `--reporter compact` يُثبّت
الصيغة فيُزال السبب، وقبول الصيغتين في القراءة فلا يعود الفشل لو تغيّرت
جملة واحدة. ومعه صار الفحص يرفع سجل الاختبارات عند الفشل، والأداة تُظهر
آخر سطور مخرَجها حين لا تعرف السبب — فالفشل الذي لا يقول لماذا نصف أداة.

## 2026-09-04 — ق-120/6: يوم البئر (هجرة 098، ق-27/ق-37/ق-39/ق-125)

### ما ثبت فعلًا

- **هجرة 098** ودالتان جديدتان: `core.well_timezone` (منطقة الجهة) و
  `core.period_bounds` (حدود الفترة بمنطقتها)، ثم `api.get_reports_summary`
  أُعيد تعريفه عليهما. `FILES=36 PASS=593` → **`FILES=37 PASS=603`**
  (+10 = عدد تحققات 098 بالضبط)، والفهرس functions 458 → **460**، و
  columns 833 وconstraints 501 وtriggers 44 بلا تغيير.
- **التحقق الحاسم:** حدّ اليوم يُحسب مرتين، ومنطقة جلسة القاعدة مضبوطة
  `UTC` أولًا ثم `America/New_York` — والنتيجة **واحدة**. فنافذة التقرير لم
  تعد تتبع من يقرأ ولا من أين، بل منطقة الجهة وحدها.
- **س-02 وس-03 مُجسَّدان في جلسة واحدة** (31 يناير 23:20 → 1 فبراير 01:40
  بتوقيت عدن): 8400 ثانية كاملة في نافذة فبراير، وصفر في يناير، والسلسلة
  اليومية تُسندها إلى `2026-02-01`. وق-37 بجلسة جارية موجودة في اليوم
  ومستثناة من المجاميع.
- **لا تعديل في العميل:** الشاشة ترسل رمز الفترة ولا تحسب حدودًا ولا
  تستعمل `custom` — فالعطب كان كله على الخادم، وتوصيف البند القديم
  («النافذة يرسلها العميل») كان متقادمًا.

### اختبارٌ كان يُرسّخ العطب

التحقق 28 في اختبار 092 كان يشترط حرفيًّا
`period_start = date_trunc('day', now())` — أي يُثبّت أن حدّ اليوم منتصف ليل
الخادم، **فيمنع إصلاحه ويشهد بالسلامة كل يوم**. كُتب من الكود القائم لا من
القرار. حُدِّث ليقيس **الخاصية** (الحدّ يقع عند منتصف الليل بمنطقة الجهة،
والمدى يوم كامل) لا صيغة حسابها — فمقياسٌ يعيد حساب ما يقيسه يوافق الكود
دائمًا ولو كان خاطئًا. وهذا وجهٌ ثالث لعائلة الثابت 699: بعد «نجاح كاذب في
الأداة» و«صمت يُقرأ صفرًا»، **حرسٌ يحرس الخطأ**.

### غير مثبت

لم تُنشر بعد (الدمج هو ما ينشر)، ولا تشغيل على جهاز أندرويد حقيقي.

## 2026-09-04 — ق-120/7: توحيد سقف الهجرات بإزالة الرقم لا بتصحيحه

### ما ثبت فعلًا

- **أربعة عشر موضعًا حاكمًا** كانت تعلن سقفًا متقادمًا (077 أو 084 أو 087
  أو 093 والواقع 098): `docs/README.md`، `docs/PROJECT_MAP.md` (موضعان)،
  `docs/technical/INVARIANTS.md` (موضعان، أحدهما **الثابت 681** نفسه)،
  `DECISION_IMPLEMENTATION_MATRIX.md`، `SEARCH_DEDUP_ARCHITECTURE.md`،
  `ANDROID_OFFLINE_BACKGROUND_SYNC.md`، `ACTIVE_SESSION_ARCHITECTURE.md`،
  `SESSION_SETTLEMENT_ARCHITECTURE.md`، `V1_IMPLEMENTATION_SEQUENCE.md`،
  `PLATFORM_ADMIN_ACCOUNTS_WELLS_SUPPORT_ARCHITECTURE.md`،
  `RESEARCH_STANDARDS_GATE.md` (موضعان)، `AI_HANDOFF_PROTOCOL.md`، و
  `UX_UI_SPEC.md` (أربعة مواضع). كلها صارت تشير
  إلى `AGENTS.md` §4 **بلا رقم**، وذلك السطر أُعلن المصدر الوحيد للسقف
  والتالي. فالعلاج ليس تصحيح أرقامٍ تتقادم بعد هجرتين، بل إلغاء تعدّد
  المصادر: ما لا يحمل رقمًا لا يتقادم.
- **والعدّ نفسه كان مقصورًا:** الوثيقة التي تصف المسألة حصرتها في 21 موضعًا،
  وأول بحث لي اقتُطع بـ`head` فأظهر ستة. العدّ الكامل: **34 موضعًا** — 14
  حاكمة صُحِّحت، و14 في `DECISIONS.md` سجلٌّ مؤرَّخ يبقى، و6 في وصف المسألة
  نفسها. **البحث المقتطع يُقصّر المحيط كما يُقصّره الوصف سطرًا سطرًا** —
  وهذا ثاني ظهور لعائلة م-41D4 في جولة واحدة.
- **وقسم «baseline الحالي المثبت»** في `PROJECT_MAP.md` كان يُعلن نفسه
  «الحالي» بأرقام 2026-08-30 — 86 هجرة، 25 ملف اختبار، 362 نجاحًا، 34 RPC
  — بعد أن صار الواقع 97 هجرة و37 ملفًا و603 نجاحات. **نجاح كاذب في وثيقة
  لا في كود**، ويقرؤه وكيل فيبني عليه. صار يحيل إلى `RESUME_POINT` §3،
  وحدوده التي لا تحمل رقمًا (Direct DML = 0، `anon` EXECUTE = 0،
  SECURITY DEFINER داخل `api` = 0) بقيت كما هي، وقياساته القديمة أُعلنت
  تاريخيةً بتاريخها.
- **والسجلات لم تُحرَّف:** `PROGRESS.md` و`DOC_CHANGELOG.md` و
  `RESUME_HISTORY.md` تحمل أرقامًا كانت صحيحة لحظة كتابتها، فتوحيد الأرقام
  لا يعني تحريف التاريخ.
- **وما بقي بقصد:** `Migration 085+` حيث تعني «هجرة لاحقة» لا رقم بدء، و
  `Migration 083+084` حيث تشير إلى هجرتين بعينهما — وكلتاهما صحيحة دائمًا.

### نشر 098

`PR #35` اجتاز الحرس (القاعدة 6m26s، التطبيق 1m32s) ودُمج. والتحقق السحابي
**97/97** و`MISSING_IN_CLOUD=0` و`FUNCTIONS_INDEX=447` مقابل `449` بلا نقص،
و`43`/`79` بلا تغيير. وأول قياس بعد الدمج بثوانٍ أظهر سطر الهجرة ناقصًا
**ودوالّها كاملة** — تأخّر سجلّ لا عطب نشر، والأداة تُعلنه «فشلًا»: فشل كاذب
ينتظر تمييزًا.

NEXT = تقرير إغلاق ق-120، وفيه قرار للمالك: البند 5 (استثناء دوال الإضافات
عبر `pg_depend`) والحرس غير المانع (شرط حماية `main` في الموقع) — يُغلقان
أولًا أم تُغلق البوابة باستثناءين معلنين.
