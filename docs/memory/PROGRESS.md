...
**مثبت 2026-09-02:** `db:reset` + `db:test` عند المالك — هدف 093 =
`PASS=15 FAIL=0 ERROR=0`، والحزمة `FILES=31 PASS=500 FAIL=0 ERROR=0`،
و`db:index` أعاد توليد الفهرس. الالتزام `7f2a517` على الفرع
`feat/m-41d7-price-read` وPR #20.

**غير مثبت:** الدمج والنشر — دمج `main` ينشر الهجرة على الإنتاج
تلقائيًا، وأثرها هناك توسيع اطلاع لكل مشغل ومدير مُعيَّن نشط؛
والتحقق السحابي (`cloud:verify`) لم يُشغَّل بعد.
