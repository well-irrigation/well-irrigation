...
هجرة **095** واختبارها بـ26 تحققًا: أرقام القاعدة (`FILES` و`PASS` و
`db:index`) تُكتب بعد `npm run c:db` لا قبله. والنشر السحابي بعد الدمج.

**أول تشغيل قاعدة فشل وأُصلح (2026-09-03):** `db:reset OK` ثم
`db:test FAIL` عند `082_permission_enforcement_ops` بـ`PASS=19 FAIL=1`،
والسطر: «بقي 1 دالة على السلطة القديمة». السبب: القارئ الداخلي
`finance.read_partner_overview` كان يقبل `iam.has_well_role` ليرى المالك ما
يراه شريكه، وحرس م-18 يشترط صفر دوال على مصفوفات الأدوار النصية في مخططات
العمل السبعة. الفرع حُذف — لا نُقل إلى مخطط غير محروس — وأُضيف تحقق يعيد
فحص الحرس داخل اختبار 095.

**والتشغيل الثاني فشل بنفس السطر:** الحرس يقرأ `pg_get_functiondef` بنصّه
كاملًا **بتعليقاته**، والتعليق الذي يشرح الحذف كان يحمل اسم النمط الممنوع
فيُعَدّ كأنه هو. الشرح انتقل فوق الدالة لا داخلها. **والتشغيل لم يُعَد بعد.**
