...
وكذلك تُركت — بتسجيل صريح لا بإغفال — ست وثائق معمارية
تحمل في ترويستها «أول DB Migration جديدة: 078 أو أحدث»:
`SEARCH_DEDUP` و`ACTIVE_SESSION` و`SESSION_SETTLEMENT` و
`OPERATIONS_RECORDS` و`MONEY_PARTNERS` و
`WELL_MANAGEMENT_REPORTING`. القاعدة خاطئة منذ تطبيق 078
فعلًا وليست من إنتاج هذه الجولة، والخطر حقيقي لا تجميلي:
من يتبعها يبدأ هجرة تصطدم بملف مختوم. الصواب = 085 أو أحدث.
لم تُعدَّل هنا لأن تعديلها يستلزم إعادة تأريخ ست وثائق داخل
commit خاص بحماية التكرار؛ سُجّلت في `RESUME_POINT.md`
كأول عنصر في جولة التنظيف المؤجلة.
