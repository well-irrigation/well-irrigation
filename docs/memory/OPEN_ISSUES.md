...
1. ~~**092 غير مُتحقَّقة على القاعدة**~~ — **أُغلق في 2026-09-02**:
   ...
   بالدمج في `main`.
2. **معرّف دفعة مُلفَّق في مسار كتابة** — تفصيله أدناه.
...
### معرّف دفعة مُلفَّق في «استخدام الرصيد المقدم»
...
- **البند 8** (`mock-advance-pay` في
  `farmer_financial_account_screen.dart:708`) كما هو — الموضع الوحيد
  الباقي من دين المعرّفات المُلفَّقة، ويحتاج عقد قاعدة (ق-99).
