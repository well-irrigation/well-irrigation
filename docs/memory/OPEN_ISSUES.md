...
- م-38: **Implemented** — منح تنفيذ داخلي محدود في Migration 087؛ **Verified local** باختبار الصلاحيات البنيوي، ولم يُتحقق سحابيًا.
- م-39: **Implemented** — أزيلت عملية ×100؛ اختبار Flutter مضاف، ولم يُشغّل بسبب بيئة Flutter للقراءة فقط.
- م-40: **Implemented** — فشل backend أصبح فشلًا صريحًا بلا completion أو إغلاق ناجح؛ اختبار سلوك واجهة كامل مؤجل لبيئة Flutter قابلة للتشغيل.
...
**الحالة:** مفتوحة — فجوة مؤكدة — 2026-08-30
**الأولوية:** P0 قبل أي توسع وظيفي
**القرار الحاكم:** ق-78، ق-79، ق-120

### الواقع المثبت

- غلاف `api.setup_well_full` يعمل بصفة `SECURITY INVOKER`.
- المستخدم `authenticated` يستطيع استدعاء غلاف `api`.
- `core.setup_well_full` ليست قابلة للتنفيذ لهذا المستخدم.
- النتيجة المثبتة في البيئة الحية: `permission denied`.
- إنشاء البئر يفشل فعليًا.

### شرط الإغلاق

إصلاح الصلاحيات بما يطابق API architecture، ثم اختبار صلاحيات
المستخدم والرفض المقصود للطبقة الداخلية، مع Regression Test.

لا تعتبر هذه المسألة منفذة أو متحققة قبل الدليل.
...
- Flutter يعرض ويدخل السعر بوحدة `YER/hour`.
- ق-77 وق-119 تحكمان بأن اللاحقة `_minor` تاريخية، والقيمة ريال
  يمني كامل.
- `create_well_wizard_screen.dart` يضرب القيم في `×100`.
- `core.setup_well_full` يخزن القيم مباشرة.
- المثال المثبت: `3500` تصبح `350000`.
- لم تتأثر بيانات حقيقية لأن المشروع Pre-Production.

### شرط الإغلاق

توحيد الوحدة بين Flutter وAPI والتخزين، وإضافة Regression Test
يثبت أن `3500` تبقى `3500`.
...
- عند فشل إنشاء البئر في Backend يمسك التطبيق الخطأ.
- بعد ذلك يعرض أن البيانات حُفظت محليًا ويكمل التدفق.
- لا يوجد في هذا المسار دليل على حفظ محلي حقيقي.
- لا يجوز عرض نجاح غير مثبت.

### شرط الإغلاق

إزالة النجاح الصامت، أو ربطه بحفظ محلي حقيقي مثبت مع حالة
واضحة (`local_only` / `pending`) واختبار فشل واستعادة.
