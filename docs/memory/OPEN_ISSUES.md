...
**حالة الجولة:** ✅ التحقق المحلي **مثبت 2026-09-02** — هدف 093 =
15/15، والحزمة `FILES=31 PASS=500 FAIL=0 ERROR=0`، و`db:index` أعاد
التوليد. بقي **الدمج** (PR #20 على الالتزام `7f2a517`)، ودمجه في
`main` ينشر الهجرة على الإنتاج تلقائيًا، وأثرها هناك **توسيع اطلاع**:
كل مشغل ومدير مُعيَّن نشط يرى تسعيرة بئره بعد الدمج. والتحقق السحابي
(`cloud:verify`) بعد الدمج.
