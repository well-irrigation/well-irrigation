...
3. **حماية `main` قبل الدمج** — `main` موصول بقاعدة الإنتاج، فدمج
   092 ينشرها تلقائيًا هناك بلا تشغيل اختبارات. ضابط الحماية في
   `GitHub` كان آخر مرة **غير محفوظ** (`Create` لم يُضغط).
...
- **المشغل لا يرى التسعيرة أصلًا**: العقد `api.get_active_price_schedule`
  يفحص `price.manage`، وهي للمالك وحده. فمن يشغّل البئر يرى لافتة
  الصلاحية ولا يرى سعر الساعة. إن كان المطلوب أن يراه فذلك صلاحية قراءة
  جديدة (`price.read` تُمنح للمشغل والمدير) = Migration جديد تمنعه
  ق-120، فيبقى قرارًا مفتوحًا لا دينًا مخفيًّا.
