...
1. **089 و090 و091 غير منشورة سحابيًا** — التحقق محلي فقط، والنشر
   بيد المالك عبر قناة `6543`. أما محليًا فقد نجح `db:reset` و
   `db:test` (091 = **PASS=34**، الحزمة = **448 PASS / 0 FAIL**).
2. **`commit ba4176c` دُفع إلى `main` مباشرة** بلا `PR`، فلا
   مراجعة مسجَّلة لـم-41D1. الجولة التالية تبدأ من فرع
   `fix/m41d2-finance-boundary`.
3. **حرّاس عدد عُدِّلت عن قصد في الحقبة المقفلة**: 080 و081
   (الفهرس 39→41، الإجمالي 73→75). أي مراجعة يجب أن تتحقق أن
   السبب هو رمزا الصلاحية الجديدان لا انزلاق غير مقصود.
4. **`recordFuelPurchase` ضاق سلوكه**: العقد القائم
   `api.purchase_fuel` يأخذ البئر لا الخزان، ولا يقبل اسم مورّد
   ولا ملاحظة. الحقلان أُزيلا من الشاشة وشُرح السبب فيها. توسيع
   العقد قرار مستقل.
5. **عقد حدود اليوم على الخادم** — ما زال مفتوحًا كما هو.
6. **Bare RPC debt = 1** (`get_reports_summary`، لا عقد له بعد) و
   **dotted-from debt = 5** (كلها `finance_repository`).
7. الفجوات السابقة كما هي: إدارة الفريق، Auth/OTP،
   settings false-success، Integration/E2E، CI/branch protection.
