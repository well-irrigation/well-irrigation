...
**الحالة:** مفتوحة — **ضُيِّقت 2026-08-22 بق-114 / Migration 083+084**
**القرارات الحاكمة:** ق-89، ق-90، ق-114
...
لكل عملية ميدانية لا لكل محاولة إرسال. الهاتف لم يُبنَ بعد،
فلا يوجد من يولّد هذا المعرّف ولا من يخزّن العملية محليًا
حتى يعيد إرسالها.

أي أن الخادم صار جاهزًا لاستقبال إعادة إرسال آمنة، ولم
يُبنَ بعد ما يعيد الإرسال.

### المتبقي

- Local durable database.
- ordered outbox.
- **stable command IDs** — التوليد والتخزين والتثبيت عبر
  محاولات الإرسال؛ الخادم صار يستقبلها.
- ~~idempotent API replay~~ — **منفَّذ خادميًا** للثماني
  عمليات؛ يبقى إثباته من الهاتف فعليًا.
